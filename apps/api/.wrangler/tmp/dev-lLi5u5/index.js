...
// .wrangler/tmp/bundle-2Ni32D/strip-cf-connecting-ip-header.js
...
var src_default = {
  async fetch(request, env, ctx) {
    ...
    const corsHeaders = {
      ...
      // Em prod, trocar pelo domínio do Pages
    ...
    const getAuthUser = /* @__PURE__ */ __name(async () => {
    ...
    if (url.pathname === "/api/auth/login" && request.method === "POST") {
      try {
        ...
        if (!email || !password) {
          ...
          return new Response(JSON.stringify({ error: "Email e senha s\xE3o obrigat\xF3rios" }), {
            status: 400,
            headers: { ...corsHeaders, "Content-Type": "application/json" }
          });
        }
        ...
        const token = await new SignJWT({ id: user.id, email: user.email }).setProtectedHeader({ alg: "HS256" }).setExpirationTime("24h").sign(new TextEncoder().encode(env.JWT_SECRET));
    ...
    if (url.pathname === "/api/oauth/tiktok/callback") {
      ...
      try {
        ...
        `).bind(
          ...
          access_token,
          refresh_token,
          Math.floor(Date.now() / 1e3) + expires_in
        ).run();
        ctx.waitUntil((async () => {
          try {
    ...
    if (url.pathname === "/api/jobs/publish" && request.method === "POST") {
      ...
      const { accounts, campaign } = await request.json();
      const workspace_id = "ws_123";
      ...
      return new Response(JSON.stringify({
        job_id,
        message: "Job de publica\xE7\xE3o enfileirado"
      }), {
        headers: { ...corsHeaders, "Content-Type": "application/json" }
      });
    }
  ...
  async queue(batch, env) {
    for (const message2 of batch.messages) {
      const { job_id } = message2.body;
      await env.DB.prepare("UPDATE publish_jobs SET status = 'processing' WHERE id = ?").bind(job_id).run();
      const job = await env.DB.prepare("SELECT * FROM publish_jobs WHERE id = ?").bind(job_id).first();
      const items = (await env.DB.prepare("SELECT * FROM publish_job_items WHERE job_id = ?").bind(job_id).all()).results;
...
// .wrangler/tmp/bundle-2Ni32D/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-2Ni32D/middleware-loader.entry.ts
